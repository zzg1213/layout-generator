...
def build_active_cells(
    ...
    if layout_type not in (None, "full", "layout3"):
    ...
    if num_spans_x < 4 or num_spans_y < 3:
        raise ValueError("layout3 requires at least 4 X spans and 3 Y spans.")
    notch_depth = min(2, max(1, int(math.floor(num_spans_x * MAX_REENTRANT_RATIO))))
    for j in (0, num_spans_y - 1):
        for i in range(num_spans_x - notch_depth, num_spans_x):
            cells.discard((i, j))
    return cells
