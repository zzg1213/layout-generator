...
def build_active_cells(
    ...
    if layout_type not in (None, "full", "layout6"):
    ...
    if num_spans_x < 4 or num_spans_y < 3:
        raise ValueError("layout6 requires at least 4 X spans and 3 Y spans.")
    notch_x = min(2, max(1, int(math.floor(num_spans_x * MAX_REENTRANT_RATIO))))
    notch_y = min(2, max(1, int(math.floor(num_spans_y * MAX_REENTRANT_RATIO))))
    for j in range(num_spans_y - notch_y, num_spans_y):
        for i in range(0, notch_x):
            cells.discard((i, j))
    for j in range(0, notch_y):
        for i in range(num_spans_x - notch_x, num_spans_x):
            cells.discard((i, j))
    return cells
