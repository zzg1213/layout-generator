...
def build_active_cells(
    ...
    if layout_type not in (None, "full", "layout8"):
    ...
    if num_spans_x < 4 or num_spans_y < 3:
        raise ValueError("layout8 requires at least 4 X spans and 3 Y spans.")
    hole_w = 1 if num_spans_x < 6 else 2
    hole_h = 1 if num_spans_y < 5 else 2
    start_i = (num_spans_x - hole_w) // 2
    start_j = (num_spans_y - hole_h) // 2
    for j in range(start_j, start_j + hole_h):
        for i in range(start_i, start_i + hole_w):
            cells.discard((i, j))
    return cells
