...
def build_active_cells(
    ...
    if layout_type not in (None, "full", "layout2"):
    ...
    if num_spans_x < 4 or num_spans_y < 3:
        raise ValueError("layout2 requires at least 4 X spans and 3 Y spans.")
    notch_width = min(2, max(1, int(math.floor(num_spans_x * MAX_REENTRANT_RATIO))))
    start_i = max(1, (num_spans_x - notch_width) // 2)
    for j in (0, num_spans_y - 1):
        for i in range(start_i, start_i + notch_width):
            cells.discard((i, j))
    return cells
